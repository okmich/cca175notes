...
val tagDF = sqlContext.read.table("cca_hive.tag")

//use sql to process the data
//alternative 1
val resultDF = tagDF.groupBy("movieid").count

//alternative 2
tagDF.registerTempTable("tag_temp")
val resultDF = sqlContext.sql("select movieid, count(1) from tag_temp group by movieid")
